function [] = Gpufit_speed_max_threads()
...
n_fits_per_block = [];
min_threads = [];
max_threads = [32, 64, 128, 256, 512, 1024];
    ...
    for repetition = 1:n_timing_repetitions_gpufit
        [parameters_gpufit, converged_gpufit, chisquare_gpufit, n_iterations_gpufit, time_gpufit, info_gpufit(i,:)]...
            = gpufit(tmp_data, weights, model_id, tmp_initial_params, tolerance, ...
                     ...
                     gpu_memory_fraction,n_fits_per_block, min_threads, max_threads(i));
...
test1 = test;
if exist('test_max_threads.mat', 'file') == 2
    save('test_max_threads.mat', 'test1', '-append');
else
    save('test_max_threads.mat', 'test1');
end

%% plot
Plot_gpufit_speed(max_threads, speed_gpufit)

end

function [] = Plot_gpufit_speed(max_threads, speed_gpufit)

figure('Name','gpufit, variable minimum number of threads','NumberTitle','off');
plot(...
    max_threads, speed_gpufit, 'red.-', 'LineWidth', 8)
xlabel('minimum number of threads')
