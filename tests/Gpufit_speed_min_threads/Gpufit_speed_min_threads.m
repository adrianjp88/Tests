function [] = Gpufit_speed_min_threads()
...
gpu_memory_fraction = [];
n_fits_per_block = [];
min_threads = [16, 32, 64, 128, 256, 512];
max_threads = [];
    ...
    for repetition = 1:n_timing_repetitions_gpufit
        [parameters_gpufit, converged_gpufit, chisquare_gpufit, n_iterations_gpufit, time_gpufit, info_gpufit(i,:)]...
            = gpufit(tmp_data, weights, model_id, tmp_initial_params, tolerance, ...
                     ...
                     gpu_memory_fraction,n_fits_per_block, min_threads(i), max_threads);
...
test1 = test;
if exist('test_min_threads.mat', 'file') == 2
    save('test_min_threads.mat', 'test1', '-append');
else
    save('test_min_threads.mat', 'test1');
end

%% plot
Plot_gpufit_speed(min_threads, speed_gpufit)

end

function [] = Plot_gpufit_speed(min_threads, speed_gpufit)

figure('Name','gpufit, variable minimum number of threads','NumberTitle','off');
plot(...
    min_threads, speed_gpufit, 'red.-', 'LineWidth', 8)
